...
def run_master_trainer():
    print("[Rank 0] 初始化 Expert Iteration (EI) 训练环境...")
    ...
    #EI超参数(为了快速跑通，调小了参数)
    n_ei_steps = 3  # EI 的大循环次数 (讲义建议5)
    G = 4  #每道题生成多少个回答 (讲义建议8)
    questions_per_step = 64  # 每次迭代抽取的题目数量 (讲义建议 512+ )
    sft_epochs_per_step = 2  # 每次收集到正确数据后，微调几个Epoch
    ...
    val_raw = load_jsonl("data/gsm8k/test.jsonl")[:50]  # 截断验证集，保持 50 条快速验证
    ...
    for ei_step in range(1, n_ei_steps + 1):
        ...
        print(f"[Rank 0]开始Expert Iteration Step {ei_step}/{n_ei_steps} ")

        #采样题目并构造 Prompt
        #从题库中随机抽题
        batch_raw = random.sample(train_raw, questions_per_step)
        batch_questions = [ex["question"] for ex in batch_raw]
        batch_gts = [
            ex["answer"].split("####")[1].strip() if "####" in ex["answer"] else ex["answer"].strip()
            for ex in batch_raw
        ]

        #为了让vLLM生成G个回答，我们最简单的做法就是把Prompt复制G遍
        flat_prompts = []
        flat_gts = []
        for q, gt in zip(batch_questions, batch_gts):
            formatted_q = prompt_template.replace("{question}", q)
            flat_prompts.extend([formatted_q] * G)
            flat_gts.extend([gt] * G)

        #让副机(vLLM)生成回答
        print(f"[Rank 0]发送 {len(flat_prompts)} 条生成任务给副机(带有温度采样)...")
        dist.broadcast_object_list(["EI_GENERATE"], src=0)
        ...
        #判分与过滤
        print("[Rank 0]收到生成结果，开始判分与过滤...")
        filtered_sft_data = []

        for prompt, resp, gt in zip(flat_prompts, flat_responses, flat_gts):
            reward_dict = r1_zero_reward_fn(resp, gt)
            if reward_dict.get("answer_reward", 0.0) > 0.0:
                #截断多余字符。只要拿到满分，说明里面一定有</answer>。
                #把之后的文字切掉，保持训练数据的绝对纯净。
                clean_resp = resp.split("</answer>")[0] + "</answer>" if "</answer>" in resp else resp
                filtered_sft_data.append({
                    "prompt": prompt,
                    "response": clean_resp
                })

        print(f"[Rank 0]EI过滤完成: {len(filtered_sft_data)} / {len(flat_prompts)} 个回答完全正确！")

        if len(filtered_sft_data) == 0:
            print("[Rank 0]本轮没有生成任何正确答案，跳过微调阶段...")
            continue

        #在自己生成的正确轨迹上微调
        print(f"[Rank 0]开始在 {len(filtered_sft_data)} 条正确轨迹上进行 SFT 微调...")
        policy_model.train()
        optimizer.zero_grad()
        micro_step = 0

        for epoch in range(sft_epochs_per_step):
            random.shuffle(filtered_sft_data)

            for idx in range(0, len(filtered_sft_data), micro_batch_size):
                batch = filtered_sft_data[idx: idx + micro_batch_size]
                ...
                if micro_step % gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                    print(f"[Rank 0]SFT Global Step {global_step}, Loss: {meta['loss'].item():.4f}")

        #如果跑完Epoch还有没step的梯度，强制step一下
        if micro_step % gradient_accumulation_steps != 0:
            nn_utils.clip_grad_norm_(policy_model.parameters(), 1.0)
            optimizer.step()
            optimizer.zero_grad()
            global_step += 1
            print(f"[Rank 0]SFT Global Step {global_step} (Remainder), Loss: {meta['loss'].item():.4f}")

        #周期性验证(Eval)
        print(f"\n[Rank 0]EI Step {ei_step}结束，触发验证 ===")
        ...
        correct_count = 0
        for i, (gt, a) in enumerate(zip(val_ground_truths, generated_texts)):
            reward_dict = r1_zero_reward_fn(a, gt)
            if reward_dict.get("answer_reward", 0.0) > 0.0:
                correct_count += 1

        accuracy = correct_count / len(val_questions)
        print(f"[Rank 0]EI验证准确率: {accuracy * 100:.2f}% ({correct_count}/{len(val_questions)})")

    print("[Rank 0]EI训练完成，发送 STOP 信号...")
...
def run_worker_generator(vllm_model):
    ...
    #EI收集数据阶段的配置(min_tokens=4避免空字符串)
    ...
    while True:
        ...
        if cmd == "EI_GENERATE":
            ...
            print(f"[Rank 1]使用vLLM生成{len(prompts)}条EI回复...")
            outputs = vllm_model.generate(prompts, sampling_params_ei, use_tqdm=False)
